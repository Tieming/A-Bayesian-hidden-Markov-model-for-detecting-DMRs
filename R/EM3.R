...
# Tieming, 7/25/2017
#
# EM3.R: (1) requires equal absolute values of mu+ and mu-;
#            (2) requires equal values of sd0, tao1, and tao2.
#
#

#' EM Algorithm for HMM 
#'
#' This function implements the EM algorithm for estimating HMM parameters.
#'
#' @param initial.value Initial values for EM algorithm. This is the output from initial.value function
...
## EM algorithm for finding the best chain of hidden states
## parameter values after convergence.
    ...
    repeat{
	    ...
	    ## M-step: update parameter values to maximize the goal function;
	    ## i.e., the expected log likelihood.
	    para.new <- m.step(obs, para.current, e.loglikhood)
	 	para.new
	 	    
	    ## set a tolerance threshold for checking convergence.
	    tol <- 0.001
	    if(abs(para.current$mu.pos-para.new$mu.pos) < tol &
	       abs(para.current$mu.neg-para.new$mu.neg) < tol &
	       abs(para.current$sd0 - para.new$sd0) < tol &
	       abs(para.current$tao1 - para.new$tao1) < tol &
	       abs(para.current$tao2 - para.new$tao2) < tol &
	...
	return(list(state=state, para=para.current))
	
}


m.step <- function(obs, para.current, e.loglikhood) {
    ## M step: maximize the expected log-likelihood in the E step.
...
update.p <- function(hk.init, dist, P_kl, rho, k) {
    ...
    return(term1 + term2)
    # maximize (term1+term2) = minimize(-term1-term2).
}
...
## E step: compute the expected log-likelihood.
    ...
    f <- NULL    # f: stores forward likelihood.
    mu.pos <- para$mu.pos  # prior mean for hyper hidden state (k=1).
    mu.neg <- para$mu.neg   # prior mean for hypo hidden state (k=2).
    sd0 <- para$sd0  # sd for normal hidden state(k=0).
    tao1 <- para$tao1  # sd for hyper hidden state(k=1).
    tao2 <- para$tao2  # sd for hypo hidden state(k=2).
    bin.cnt <- 1
    ...
    repeat{ # recursion to compute f matrix.
          bin.cnt <- bin.cnt + 1
    ...
    ## backward algorithm.
    bin.cnt <- B
    b <- NULL  # b: stores backward likelihood.
	b <- rbind(b, c(1,1,1))
	repeat{  # recursion to compute b matrix.
		    bin.cnt <- bin.cnt - 1
  	...
  	## Combine foward and back algorithm to compute
  	## P_k = P(S_b=k| O, para) and P_kl*log(a_kl) = P(S_b=k, S_[b+1]=l | O, para)*log(a_kl).
  	...
  	## P_kl_a_kl : dim(B-1, 9); the 9 columns are: k=1, l=1; k=1, l=2; k=1, l=3; ...; k=3, l=3.
  	## sum_P_kl_a_kl : sum of P_kl_a_kl across all (B-1) rows and 9 columns.
  	sum_P_k_logOb <- 0
  	## sum_P_k_logOb : sum of P_k * log (P (O_b|S_b=k)) across B rows and 3 columns.
  	for(bin.cnt in 1:(B-1)){
  		
  		# compute sum_P_kl_loga_kl.
        ...
        P_kl_temp <- NULL
        for(i in 1:3){
        	if(sd0==0 & obs[bin.cnt+1,1]==0){
        		 P_kl_temp <- c(P_kl_temp, 
                                      f[bin.cnt,i]*tran.a[i,]*
                                      c(1, 
                                      dnorm(obs[bin.cnt+1,1], 
                                      mean=c(mu.pos, mu.neg), sd=c(tao1,tao2)))*b[bin.cnt+1,])
        	}else if(tao1==0 & obs[bin.cnt+1,1]==mu.pos){
        		 P_kl_temp <- c(P_kl_temp, 
                                      f[bin.cnt,i]*tran.a[i,]*
                                      c(dnorm(obs[bin.cnt+1,1], 0, sd0), 1,
                                      dnorm(obs[bin.cnt+1,1], mu.neg, tao2))*
                                      b[bin.cnt+1,])
        	}else if(tao2==0 & obs[bin.cnt+1,1]==mu.neg){
        		P_kl_temp <- c(P_kl_temp, 
                                      f[bin.cnt,i]*tran.a[i,]*
                                      c(dnorm(obs[bin.cnt+1,1], 0, sd0),
                                      dnorm(obs[bin.cnt+1,1], mu.pos, tao1), 1)*
                                      b[bin.cnt+1,])
        	}else{
               P_kl_temp <- c(P_kl_temp, 
  	    ...
  	    # compute sum_P_k_logOb.
  	    if(sd0==0 & obs[bin.cnt,1]==0){
  	            Ob <- c(1, dnorm(obs[bin.cnt,1], mu.pos, tao1), 
  	                            dnorm(obs[bin.cnt,1], mu.neg, tao2))
  	    }else if(tao1==0 & obs[bin.cnt,1]==mu.pos){
  	            Ob <- c(dnorm(obs[bin.cnt,1], 0, sd0), 1, 
  	                         dnorm(obs[bin.cnt,1], mu.neg, tao2))
  	    	
  	    }else if(tao2==0 & obs[bin.cnt,1]==mu.neg){
  	    	    Ob <- c(dnorm(obs[bin.cnt,1], 0, sd0),
  	                         dnorm(obs[bin.cnt,1], mu.pos, tao1), 1)
...
## Forward algorithm: based on f(b-1, k) to compute f(b, k),
## where b=bin b, and k=0, 1, 2 three hidden states.
    tran.p <- para$tran.p
    rho <- para$rho
    o.b <- obs[1]
    dist.b <- obs[2]

    ## compute f(b-1,l)*a(b-1,l to k), where k and l=0, 1, 2 three hidden states.
    ...
    ## compute f(b-1,l)*a(b-1,l to k)*P(O_b|S_b=k).
    mu.pos <- para$mu.pos  # prior mean for hyper hidden state (k=1).
    mu.neg <- para$mu.neg   # prior mean for hypo hidden state (k=2).
    sd0 <- para$sd0  # sd for normal hidden state(k=0).
    tao1 <- para$tao1  # sd for hyper hidden state(k=1).
...
## Backward algorithm: based on b(b+1, k) to compute b(b, k),
## where b=bin b, and k=0, 1, 2 three hidden states.	
	tran.p <- para$tran.p
	rho <- para$rho
	o.b <- obs[1]
	...
	## compute a(b=k,b+1=l)*P(O_[b+1]|S_[b+1]=l).
	mu.pos <- para$mu.pos  # prior mean for hyper hidden state (k=1).
    mu.neg <- para$mu.neg   # prior mean for hypo hidden state (k=2).
    sd0 <- para$sd0  # sd for normal hidden state(k=0).
    tao1 <- para$tao1  # sd for hyper hidden state(k=1).
	...
	## compute a(b=k,b+1=l)*P(O_[b+1]|S_[b+1]=l)*b(b+1,l).
	if(sd0==0 & o.b==0){
	        b.bin.b <- rbind(tran.a[,1]*1,
